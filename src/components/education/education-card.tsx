...
export function EducationCard({ material, locale }: { material: EducationMaterial; locale: string }) {
  return (
    <Link
      href={`/education/${material.contentType === 'video' ? 'videos' : 'seminars'}/${material.slug}`}
        ...
        <div className="mt-auto flex flex-wrap items-center gap-4 pt-2 text-sm text-[#1A1A1A]/70">
          {material.level ? (
            <span className="inline-flex items-center gap-1.5">
              ...
              Уровень: {MATERIAL_LEVEL_LABELS[material.level]}
            </span>
          ) : null}
          {material.format ? <FormatBadge format={material.format} /> : null}
        </div>
      </div>
    </Link>
  )
